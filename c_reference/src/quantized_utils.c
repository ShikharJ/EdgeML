...
void q15_v_sub_scalar(const Q15_T* vec, Q15_T scalar, ITER_T len, Q15_T* ret,
...
void q15_v_relu(const Q15_T* vec, ITER_T len, Q15_T* ret) {
  for (ITER_T i = 0; i < len; i += 4) {
    *ret++ = q15_relu(*vec++, Q15_TMAX);
    *ret++ = q15_relu(*vec++, Q15_TMAX);
    *ret++ = q15_relu(*vec++, Q15_TMAX);
    *ret++ = q15_relu(*vec++, Q15_TMAX);
  }
